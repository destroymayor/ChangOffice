...
export default class Charges extends Component {
    ...
    this.state = {
      chargesdata: [
        ...
        },
        {
          ...
          CopyMode: "翻拍",
          Copyformat:
            "3X5吋：每張八十元、4X6吋：每張一百元、5X7吋：每張一百五十元、8X10吋：每張一百八十元、10X12吋：每張六百元、11X14吋：每張七百五十元、16X20吋：每張九百元",
        ...
        },
        {
          ...
          Copyformat: "B4(含)尺寸以下：每張二元、A3尺寸：每張三元",
          Remarks:
            "電子檔案係指圖像及文字影像檔。紙張列印輸出如為彩色列印，以左列黑白複製收費標準五倍計算；相紙黑白、彩色列印輸出之收費標準相同。電子儲存媒體離線交付費用不含儲存媒體本身之費用。*本所暫不提供此項服務。"
        },
        {
          line: 4,
          FileApearanceType: "電子檔案",
  ...
  renderRow({ item }) {
    return (
      <View style={{ width: "100%", marginBottom: 10, backgroundColor: "rgba(255,255,255,0.5)" }}>
        <Text style={{ margin: 5, fontSize: 17 }}>
          檔案外觀型式：
          {item.FileApearanceType}
        </Text>
        <Text style={{ marginLeft: 5, marginRight: 5, fontSize: 17 }}>
          複製方式：
          {item.CopyMode}
        </Text>
